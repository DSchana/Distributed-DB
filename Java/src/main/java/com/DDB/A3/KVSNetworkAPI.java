...
public class KVSNetworkAPI {
    ...
    public void run() {
        ...
        try (ServerSocket hostSocket = new ServerSocket(socketNumber)) {
            ...
            //Stay on to lookout for more client - essentially an infinite loop
    ...
    private static class ClientHandler implements Runnable {
        ...
        public void runCommand(String command, String payload) {
            p.println("KVSNetworkAPI RETURN"); // let's client know that return is coming
            if (command.equals("insert") || command.equals("update") || command.equals("upsert")) {
                HashMap<String, Object> data = kvsAPI.deserializeJson(payload);
                for (String key : data.keySet())
                    switch (command) {
                        case "insert":
                            p.println(kvsAPI.insert(key, data.get(key)));
                            break;
                        case "update":
                            p.println(kvsAPI.update(key, data.get(key)));
                            break;
                        default: // upsert
                            p.println(kvsAPI.upSert(key, data.get(key)));
                    }
            }
                ...
                keys = gson.fromJson(payload, String[].class);
                    for (String key : keys)
                        switch (command) {
                            case "get":
                                p.println(kvsAPI.get(key));
                                break;
                            case "delete":
                                p.println(kvsAPI.delete(key));
                                break;
                            //left value as boolean to make it easier for client to use
                            default: // for find
                                p.println(key);
                                p.println(kvsAPI.find(key));
                        }
            } 
            else //commands with no parameter
                switch (command) {
                    case "clear":
                        p.println(kvsAPI.clear());
